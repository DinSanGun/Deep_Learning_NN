...
def compute_gradients(X_out, Y_pred, Y_true):
    """
    Compute the gradients of the loss w.r.t weights and bias of the output layer

    Parameters:
    X_out: Input to the last layer (activation of previous layer) - shape (input_neurons x samples_num)
    Y_pred: Predicted probabilities (after softmax) - shape (classes_num x samples_num)
    Y_true: True labels - shape (classes_num x samples_num)

    Returns:
    dW_out: Gradient of loss w.r.t. the output layer weights matrix - shape (classes_num x hidden_neurons)
    db_out: Gradient of loss w.r.t. the output layer bias vector - shape (classes_num x 1)
    """
    samples_num = Y_true.shape[1]

    dZ_out = (Y_pred - Y_true) / samples_num  # gradient of loss w.r.t Z_out
    dW_out = dZ_out @ X_out.T  # gradient of loss w.r.t W_out
    db_out = np.sum(dZ_out, axis=1, keepdims=True)  # gradient of loss w.r.t b_out

    return dW_out, db_out, dZ_out
...
# Compute gradients for a 3 hidden layers network
def backpropagation(X, Y, activations, pre_activations_Z, layers):
    ...
    grads = [(dW1, db1, dX), (dW2, db2, dA1), (dW3, db3, dA2), (dW4, db4, dA3)]
    return grads
...
# Perform forward pass starting from layer i
