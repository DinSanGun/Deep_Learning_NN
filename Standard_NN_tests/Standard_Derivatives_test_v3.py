...
F = lambda X, W, b, Y, i, layers: jacobian_test_layer_i(X, W, b, Y, i, layers)
g_F = lambda X, Y, activations, pre_activations, layers: backpropagation(X, Y, activations, pre_activations, layers)
...
d_w = np.random.randn(output_classes, input_features)
d_x = np.random.randn(input_features, n_samples)
d_b = np.random.randn(output_classes)
d_b = d_b.reshape(-1, 1)

epsilon = 0.1
layers = initialize_network(X.shape[0], [X.shape[0], X.shape[0], X.shape[0]], Y.shape[0])
...
plt.legend()
plt.title(f"Weights Jacobian test in semilogarithmic plot - Layer {i + 1}")
plt.xlabel("k")
plt.ylabel("error")
plt.show()
...
plt.legend()
plt.title(f"Input Jacobian test in semilogarithmic plot - Layer {i + 1}")
plt.xlabel("k")
plt.ylabel("error")
plt.show()
...
plt.legend()
plt.title(f"Bias Grad test in semilogarithmic plot - Layer {i + 1}")
plt.xlabel("k")
plt.ylabel("error")
plt.show()
